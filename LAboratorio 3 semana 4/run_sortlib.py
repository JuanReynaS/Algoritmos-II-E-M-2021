...
#             línea de comando:
#
#             >./ python3 sortlib.py -t[#num] -m[#num] -s[tipo] -n[num1 num2 num3 ...] -a [alg]
#
...
def algoritmos_nlgn(lista):
    ...
    fin = time.time()
    tiempo_mer_thr = fin - inicio

    lista = lista_apoyo[:]
    inicio = time.time()
    verify_list(sl.mergesort_three(lista))
    fin = time.time()
    tiempo_mer_opt = fin - inicio

    return tiempo_mer, tiempo_mer_it, tiempo_mer_thr, tiempo_mer_opt
...
if fam_alg == "on2":
    ...
    for size in nlis:
        for x in range(intentos):
            lista_u = seleccion_lista(tip_sec, size, long_str)
            tmer, tmerit, tmerth, tmeropt = algoritmos_nlgn(lista_u)

            array_csv.append(lista_df("MergeSort", size, x, tmer))
            array_csv.append(lista_df("MergeIterativo", size, x, tmerit))
            array_csv.append(lista_df("MergeThree", size, x, tmerth))
            array_csv.append(lista_df("MergeOPT", size, x, tmeropt))
...
df_datos.to_csv('Algoritmos_' + fam_alg + '.csv', sep='\t')
print(df_datos)
