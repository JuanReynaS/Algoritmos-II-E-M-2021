...
try:
    ...
    apoyo = sys.argv[1:]
    while (apoyo[ind] not in ["-a", "-s", "-g", "-t", "-m"]):
        if int(apoyo[ind]) < 1:
...
if fam_alg == "on2":
    ...
    for size in nlis:
        for x in range(intentos):
            ...
            vector = lista_df("InsertionSort", size, x, ti)
            array_csv.append(vector)

            vector = lista_df("BubbleSort", size, x, tb)
            array_csv.append(vector)

            vector = lista_df("SelectionSort", size, x, ts)
            array_csv.append(vector)

            vector = lista_df("ShellSort", size, x, tsh)
            array_csv.append(vector)
    ...
    for size in nlis:
        for x in range(intentos):
            ...
            vector = lista_df("MergeSort", size, x, tmer)
            array_csv.append(vector)

            vector = lista_df("MergeIterativo", size, x, tmerit)
            array_csv.append(vector)
...
df_datos[["Tiempo"]] = df_datos[["Tiempo"]].astype(float)
...
if graficar:
    sns.lineplot(
        data=df_datos, x="Num. de elementos", y="Tiempo", hue="Algoritmos",
        err_style="bars")
    plt.show()
