...
def row_df(nombre, tamaño, intento, tiempo):
    list_data = []
...
try:
    ...
    if ((type(long_str) == int and
            tip_sec in ['sreal', 'sortedreal', 'invsortedreal', 'sint',
                       'invsortedint', 'sortedint'])
            or (long_str is  None and tip_sec
            in ['sstr', 'sortedstr', 'invsortedstr'])):
    ...
    print("error introdujo un valor incorrecto")

# Se crea un DataFrame con los datos generado segun la ´solicitud de cliente.
pd.set_option("max_rows", None)
df_dat = pd.DataFrame(columns=["Algoritmos", "Num. de elementos",
                               "Intento", "Tiempo"])

df_dat[["Tiempo"]] = df_dat[["Tiempo"]].astype(float)
df_dat[["Intento"]] = df_dat[["Intento"]].astype(int)
df_dat[["Num. de elementos"]] = df_dat[["Num. de elementos"]].astype(int)
...
if fam_alg == "on2":
    ...
    for size in nlis:
        for x in range(veces):
            ...
            y = df_dat.shape[0]
            df_dat.loc[2 * y] = (row_df("InsertionSort", size, x, ti))
            df_dat.loc[2 * y + 1] = (row_df("BubbleSort", size, x, tb))
            df_dat.loc[2 * y + 2] = (row_df("SelectionSort", size, x, ts))
            df_dat.loc[2 * y + 3] = (row_df("ShellSort", size, x, tsh))
            y = df_dat.shape[0] + 1
    ...
    for size in nlis:
        for x in range(veces):
            ...
            df_dat.loc[2 * y] = row_df("MergeSort", size, x, tmer)
            df_dat.loc[2 * y + 1] = row_df("MergeIterativo", size, x, tmerit)

# Imprime contenido de DataFrame..to_string(index=False)
# reindex es para cambiar etiquetas de las folas, en este caso
print(df_dat.reindex([i for i in range(df_dat.shape[0])]))


if graficar:
    sns.lineplot(
        data=df_dat, x="Num. de elementos", y="Tiempo", hue="Algoritmos",
        err_style="bars")
    plt.show()
